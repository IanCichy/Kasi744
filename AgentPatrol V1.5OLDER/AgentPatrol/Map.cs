...
public class Map
{
    ...
    // private List<Button> GraphButtons;
    private Region[] Regions;
    private int width, height, regionWidth, regionHeight, openTiles, regionCount, agentCount;
    ...
    */
    ...
    public void makeGraphView(Point loc)
    {
        ...
        GraphTiles = new Tile[Regions[regionID].getHeight(), Regions[regionID].getWidth()];

        //Get region variables
        regionWidth = Regions[regionID].getWidth();
        regionHeight = Regions[regionID].getHeight();
        int Hadj = Regions[regionID].getHeightAdjust();
        int Wadj = Regions[regionID].getWidthAdjust();

        //Loop through the tiles and put them into the 'dummy' array
        foreach (Tile t in tls)
        {
            GraphTiles[t.getxPos() - Hadj, t.getyPos() - Wadj] = (Tile)t.Clone();
        }
    ...
    */
    ...
    {
        int adjW = (int)((w * .45) / regionWidth);
        int adjH = (int)((h * .65) / regionHeight);
        int size = Math.Min(Math.Min(adjW, adjH), 100);
        foreach (Tile t in GraphTiles)
        {
            t.setSize(size, size);
        }
