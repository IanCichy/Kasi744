...
public class Map
{
	 ...
	 */
    public Map(int h, int w, int numRegions, Point[][] tileRegions, List<Agent> Agents, int algNumber)
    {
        openTiles = 0;
    ...
    public void makeGraphView(Point loc)
    {
        ...
        foreach (Tile t in GraphTiles)
        {
            if (t.getType() == 1)
            {
                Label lbl = new Label();
                lbl.Text = "(x,x)";
                lbl.CssClass = "graphViewLabelSpacer";
                lbl.Width = 50;
                containerGraphView.Controls.Add(lbl);
            ...
            {
                Label lbl = new Label();
                lbl.Text = "(" + t.getxPos() + ", " + t.getyPos() + ")";
                lbl.CssClass = "graphViewLabel";
                lbl.Width = 50;
                containerGraphView.Controls.Add(lbl);
    ...
    */
    ...
    private void drawLinesInGraph()
    {
        Point p1 = new Point(0,0);
        Point p2 = new Point(0, 0);
        foreach (Tile t1 in GraphTiles)
        {
            foreach (Tile t2 in GraphTiles)
            {
                if (isAdjacent(t1, t2))
                {
                    p1 = t1.getPointLocation();
                    p2 = t2.getPointLocation();
                }
    ...
    {
        if (t1.getPointLocation().Equals(new Point(t2.getxPos() + 1, t2.getyPos())) ||
            t1.getPointLocation().Equals(new Point(t2.getxPos() - 1, t2.getyPos())) ||
            t1.getPointLocation().Equals(new Point(t2.getxPos(), t2.getyPos() + 1)) ||
            t1.getPointLocation().Equals(new Point(t2.getxPos(), t2.getyPos() - 1)))
            return true;
        else
            return false;
    ...
    public int countStepNum(Region r, Point p, Agent a)
    {
        Point preTarget = new Point(a.getTarget().X, a.getTarget().Y);
        Point preLocation = new Point(a.getLocation().X, a.getLocation().Y);
        int step = 0;
        a.setTarget(p);
        while (!a.getTarget().Equals(a.getLocation()))
        {
            Tile t = r.findShortestPathMove(a);
            r.setAgentLocation(a, t);
            step++;
        }
        a.setTarget(preTarget);
        a.setLocation(preLocation);
        return step;
    }
    ...
    */
    ...
    {
        //int adjW = (int)(((w * .85) * .65) / width);
        //int adjH = (int)(((h * .85) * .85) / height);
        // int size = Math.Max(Math.Min(adjW, adjH), 35);
        int size = 40;
        foreach (Tile t in GraphTiles)
            t.setSize(size, size);
        containerGraphView.Width = (size * regionWidth) + (50 * regionWidth);
        containerGraphView.Height = (size * regionHeight) + (30 * regionHeight);
    }
