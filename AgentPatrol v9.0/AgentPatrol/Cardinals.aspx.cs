        ...
        for (int j = 0; j < points.Length; j++)//for each point of different region
        {
            try
            {
                ...
                if (tile.Y < 0 || tile.Y >= Width || tile.X < 0 || tile.X >= Height)
                    return "The coordinate (" + tile.X + "," + tile.Y + ") in region " + (i / 2 + 1) + " is overstep the boundary!";
                regions[i / 2][j] = tile;
            }
            catch
            {
            ...
            if (m % 2 == 0)//agent id
                ...
                try
                {
                    ...
                    if (checkAgentCoordinate(tile, i))
                    {
                        ...
                        return "Invalid agent coordinate (" + tile.X + "," + tile.Y + ") in region " + (i + 1) / 2 + " !";
                }
                catch (Exception e)
                {
        ...
        for (int i = 0; i < regions.Length; i++)
        {
            ...
            for (int j = 0; j < thisRegion.Length; j++)
                if (h.Contains(regions[i][j]))
                    return "The coordinate (" + regions[i][j].X + "," + regions[i][j].Y + ") in region " + (i + 1) + " is duplicate with another coordinate!";
                else
                    h.Add(regions[i][j]);
        }
        return message;
                ...
                if (regionsMarks[x, y - 1] == (a + 1) || regionsMarks[x, y - 1] == -1)
                {
                    if (regionsMarks[x, y + 1] == (a + 1) || regionsMarks[x, y + 1] == -1)
                    {
                        if (regionsMarks[x - 1, y] == (a + 1) || regionsMarks[x - 1, y] == -1)
                        {
                            ...
                            {
                                message = "Invaild coordinate (" + regions[a][b].X + "," + regions[a][b].Y + ") in region " + (a + 1) + ", which makes region " + (a + 1) + " and region " + regionsMarks[x + 1, y] + " to be adjacent!";
                                return message;
                            }
                        }
                        else
                    ...
                    {
                        message = "Invaild coordinate (" + regions[a][b].X + "," + regions[a][b].Y + ") in region " + (a + 1) + ", which makes region " + (a + 1) + " and region " + regionsMarks[x, y + 1] + " to be adjacent!";
                        return message;
                    }
                }
                else
