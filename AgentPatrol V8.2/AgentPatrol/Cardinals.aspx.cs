    ...
    */
    ...
    public string saveRegion(int i, string[] points)
    {
        ...
        for (int j = 0; j < points.Length; j++)//for each point of different region
        {
            try
            {
                ...
                if (numbers.Length != 2)
                    return "The No." + j + " coordinate in region " + i / 2 + " is invalid!";
                ...
                catch (FormatException) { return "The No." + j + " coordinate in region " + i / 2 + " is invalid!"; }

                if (tile.Y < 0 || tile.Y >= Width || tile.X < 0 || tile.X >= Height)
                    return "The coordinate (" + tile.X + "," + tile.Y + ") in region " + (i / 2) + " is overstep the boundary!";
                regions[i / 2][j] = tile;
            }
            catch
            {
                return "Invalid coordinate in region " + i / 2 + "!";
            }
        }
        return message;
    }
    ...
    public string saveAgent(int i, string[] agentInfo, int algorithmNumber)
    {
        ...
        for (int m = 0; m < agentInfo.Length; m++)
        {
            if (m % 2 == 0)//agent id
                ...
                {
                    return "The ID of No." + m / 2 + " agent in region " + (i - 1) / 2 + " is invalid!";
                }
    ...
    public string checkID()
    {
        ...
        for (int i = 0; i < IDs.Length; i++)
        {
            if (ht.Contains(IDs[i]))
            {
                ...
                message = "The ID of agent (" + p.X + "," + p.Y + ") is duplicate with another agent!";
                return message;
            }
            else
            {
        ...
        for (int i = 0; i < regions.Length; i++)
        {
            ...
            for (int j = 0; j < thisRegion.Length; j++)
                if (h.Contains(regions[i][j]))
                    return "The coordinate (" + regions[i][j].X + "," + regions[i][j].Y + ") in region " + i + " is duplicate with another coordinate!";
                else
                    h.Add(regions[i][j]);
        }
        return message;
    ...
    public string checkConnectivity(Point[] thisRegion, int i)
    {
        ...
        if (visit.Count != thisRegion.Length)
            message = "Coordinates are not connected in region " + i + " !";
        return message;
    }
    ...
    public String checkAdjacentRegion()
    {
        ...
        for (int a = 0; a < regions.Length; a++)
        {
            for (int b = 0; b < regions[a].Length; b++)
            {
                ...
                if (regionsMarks[x, y - 1] == a || regionsMarks[x, y - 1] == -1)
                {
                    if (regionsMarks[x, y + 1] == a || regionsMarks[x, y + 1] == -1)
                    {
                        if (regionsMarks[x - 1, y] == a || regionsMarks[x - 1, y] == -1)
                        {
                            if (regionsMarks[x + 1, y] == a || regionsMarks[x + 1, y] == -1)
                            {
                            ...
                            {
                                message = "Invaild coordinate (" + regions[a][b].X + "," + regions[a][b].Y + ") in region " + a + ", which makes region " + a + " and region " + regionsMarks[x + 1, y] + " to be adjacent!";
                                return message;
                            }
                        }
                        else
                    ...
                    {
                        message = "Invaild coordinate (" + regions[a][b].X + "," + regions[a][b].Y + ") in region " + a + ", which makes region " + a + " and region " + regionsMarks[x, y + 1] + " to be adjacent!";
                        return message;
                    }
                }
                else
