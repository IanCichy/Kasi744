    ...
    public void makeGraphView(Point loc)
    {
        ...
        for (int i = 0; i < regionHeight; i++)
        {
            ...
            for (int j = 0; j < regionWidth; j++)
            {
                ...
                {
                    t.getLocLabel().Text = "(" + (t.getxPos()+1) + ", " + (t.getyPos()+1) + ")";
        ...
        foreach (Agent A in Regions[currentRegionInGraph].getAgents())
        {
            ...
            txtb.Text += " - Pos: " + "(" + (A.getLocation().X+1) + "," + (A.getLocation().Y+1) + ")";
            txtb.Text += " - Tgt: " + "(" + (A.getTarget().X+1) + "," + (A.getTarget().Y+1) + ")";
            txtb.Width = 250;
            txtb.Enabled = false;
